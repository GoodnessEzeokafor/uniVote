...
export default class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (ElectionNetworkData) {
      const ElectionDapp = new web3.eth.Contract(
        ElectionAbi.abi,
        ElectionNetworkData.address
      ); // loads the smart contract
      // console.log(ElectionDapp)

      const electionAuthority = await ElectionDapp.methods
        .electionAuthority()
        .call(); // gets the address of the election coordinator
